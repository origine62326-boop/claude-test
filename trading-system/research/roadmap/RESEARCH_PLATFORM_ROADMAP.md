...
### ステップ4: MT4バックテストの正式再現（完了、2026-07-28。Phase5-1(EXP-002)分は2026-07-31に完了）
...
- 完了条件（EXP-001分）: 達成。DS002由来の`UNVERIFIED_OBSERVATION`が、DS001としてパイプラインを通した
  検証可能な数値になった（`currency`を除く全必須フィールド抽出成功、操作履歴184件中183件を
  正しくペアリング）
- **[2026-07-31追記] Phase5-1版(EXP-002, DS004)分も完了**: 2026-07-29にバックテスト自体は実行済み
  だったが、H002が本来検証したい「制限が実際に発動したか」の確認にはExpertsログが必須であり、
  未提供のままステップ4としては未完了だった。2026-07-31にExpertsログを受領・解析し、レポート側の
  ...
  `EXP-003_daily_loss_limit_function_test`（`research/experiments/`に事前登録案を作成済み、
  パラメータはユーザー承認待ち）で行う
...
上記5ステップすべてが完了していること。2026-07-31時点の進捗: ステップ1・4・5は完了
（ステップ4はEXP-001分に加え、2026-07-31にEXP-002/Phase5-1分のExpertsログ確認も完了）。
