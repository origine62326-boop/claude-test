...
- **[2026-08-15追記・O-004]** `EXP-007`本体の結果（PF改善0.824→0.866等）は、取引数が
  195→239件に増えた経路依存効果を含んだ状態での比較だった。`O-004`（`OBSERVATION_REGISTRY.md`）
  で、`EXP-002`の原195件のエントリーを変更せず建値移動ルールのみを反実仮想的に適用したところ、
  **公正な比較対象(167件、同一母集団)ではPFはむしろ悪化した（0.6413→0.5821）**。純損益は
  わずかに改善（+155.90）したが、これは少数の大きな勝ちトレード（主に買い側、11件・機会損失
  約$3883）を早期の小さな利益で打ち切ってしまう副作用が、多数の損失を小さな利益に変える効果と
  同程度の重みを持つため。**`EXP-007`単体で見えたPF改善は、建値移動そのものの純粋な効果だけに
  よるものとは言い切れない**（詳細は`O-004`参照）
