...
1回目の提出で正しいビルド・正しいパラメータが確認できた（`EXP-006`/`EXP-007`と異なり取り違えなし）。

- inputs行: `MaxBuyTrendDurationBars=50`、`TradingStartHour=8; TradingEndHour=22`（デフォルト）、
  `EnableLong=1; EnableShort=1`、他は`EXP-002`と同一設定を確認
- 初期化ログ: `初期化完了。Phase5-2(建値移動/トレーリング)・Phase7(取引時間フィルター)は
  未実装です。トレンド成熟度フィルター(H006/EXP-008)は買いのみMaxBuyTrendDurationBars=50
  (0=無効)で動作します。` — 正しいビルドであることを確認
- 同ログでERROR 0件、`発注成功`143件（レポートの総取引数と一致）、最大連敗制限2回発動・
  日次損失上限0回発動、`EA停止 reason=0`で正常終了
- コンパイルログでの明示的な「0 errors / 0 warnings」確認は今回も未取得のまま
- 本データを`DS012`として`DATASET_REGISTRY.md`へ登録した

### 結果集計（`analysis/parse_mt4_trades.py` / `direction_analysis.py`、および`EXP-006`/`EXP-007`と
同一の実現R計算式による再計算）

| 指標 | 買いサブセット | 売りサブセット |
|---|---|---|
| 取引数 | 89（`EXP-002`の141件から-36.9%、`O-006`の見込み-37.6%とほぼ一致） | 54（`EXP-002`と**完全一致**） |
| PF（$ベース） | 1.248 | 0.313（`EXP-002`と完全一致） |
| PF（Rベース） | 1.117 | - |
| 勝率 | 35.96% | 16.67%（`EXP-002`と完全一致） |
| 純損益 | +2295.89 | -7108.37（`EXP-002`と完全一致） |
| 期待値R（平均realized_R、n=89） | **+0.0751** | - |

**売りサブセットが`EXP-002`と取引数・PF・純損益まで完全一致した。** これは`CheckSellSignal()`が
本当に無変更であることの直接的な証拠であり、単一変更点の検証として理想的な結果である。

全体（買い+売り、143件）: PF0.75（公式値）、純損益-4812.48、最大DD 5.39%(5419.95)。全体PFが
`EXP-002`(0.76)からほぼ横ばいなのは、依然として売りが未変更のまま大きな負の寄与を続けている
ためであり、買い単体の改善が全体PFには表れにくい（母数として売りの方が$変動が大きい）。

### 判定（事前登録した`acceptance_criteria`との照合）

| 区分 | 事前登録した基準 | 結果 | 判定 |
|---|---|---|---|
| Primary Metric | 買い期待値R が `EXP-002`買いサブセット(+0.0180)を上回ること | +0.0751 | **達成**（基準の4倍以上） |
| Secondary: PF | `EXP-002`買いサブセット(1.059)を上回ることが望ましい | 1.248($)/1.117(R) | **達成** |
| Secondary: 取引回数 | 約37.6%減少見込み | 実際36.9%減少（141→89） | 見込みとほぼ一致 |
| Guardrail: 最低取引件数 | 買いのみで80件以上 | 89件 | **達成** |
| Guardrail: 最大DD | `EXP-002`(7.45%)から悪化しないこと | 5.39%（改善） | **達成** |

Primary/Secondary/Guardrailすべてを達成した。事前登録した`rejection_criteria`（Primary未達なら
HOLD）には該当しない。

## decision

`ADOPTED`（本実験の事前登録した基準に照らして。ただし下記limitationsの通り、H006自体を
「実運用可能」と断定するものではなく、真のOOS検証が別途望ましいことを明記する）

## acceptance_reason / rejection_reason

`RESEARCH_CHARTER.md`第13節の採用条件（事前登録した比較基準を実際に上回った、Guardrail
Metricsをすべて満たした、経路依存の影響が最小限であることを売りサブセットの完全一致で確認できた）
を満たすため`ADOPTED`とする。ただし本実験は`H006`が発見されたのと同一のデータ(`DS004`と重複する
期間)で実行されており、真のアウト・オブ・サンプル検証ではない。この点は事前登録時から明記済みの
既知の限界であり、`H006`全体を「検証済みで実運用可能」と扱うことは時期尚早である。
...
**[2026-08-15更新]** `COMPLETED`（実行・判定完了。判定は`ADOPTED`〔実験単体の基準に照らして〕。
`H006`全体の実運用可否は別途OOS検証が必要）
...
| data_version | `DS012`（`DATASET_REGISTRY.md`、v1、2026-08-15登録） |
...
- updated_at: 2026-08-15（MT4実行結果(DS012)を受領・検証し、判定を確定。status READY→COMPLETED、
  decision=ADOPTED〔実験単体〕。1回目の提出で正しいビルド・パラメータを確認、売りサブセットが
  `EXP-002`と完全一致することも確認）
