...
6. **[2026-07-28 解消] MT4バックテストレポート解析パイプラインが実データで未検証のまま継続していた。**
   `trading-system/reports/raw/`は`.gitkeep`のみで、実際の`.htm`レポートが投入されていなかった。
   これはv0.2.0リリースノートの時点から「最優先のTODO」として申し送りされていた。
   → 対応: ユーザーから実際の`.htm`レポートの提供を受け、`DATASET_REGISTRY.md`のDS001として
   正式登録(`status=ACTIVE`)した。この検証の過程で`analysis/parse_mt4_report.py`等に6件の
   実装バグ（文字コード固定、ラベル表記ゆれ未対応、行修飾語の分離未対応、連勝/連敗セルの
   主値/副値逆転、操作履歴のcolspan省略未対応等）が見つかり、修正した
   （`research/versions/BACKTEST_REPRODUCIBILITY.md`の2026-07-28追記に詳細）。
   `EXP-001`(H001)を実データで実行し、HOLD判定（サンプル数不足のため参考外）となった。

6b. **[2026-07-28新規] レポート公式値とトレード明細再計算値の乖離が未解明のまま残っている。**
    DS001について、`scripts/run_analysis.py`の`_cross_check_mismatches`が、期待利得
    (公式-33.31 vs 再計算-33.808)・純利益(公式-6128.31 vs 再計算-6186.90)・勝数(公式55 vs
    再計算54)の乖離を自動検出した。原因は未特定（テスト終了時点の未決済ポジションの含み損益が
    公式値側にのみ反映されている可能性が仮説段階）。
    → 対応: `DATASET_REGISTRY.md` DS001、`EXP-001`の`limitations`に記録済み。原因調査は
    優先度P2として次の作業候補に残す（現時点でH001の判定(HOLD)はサンプル数不足が主因であり、
    この乖離自体が結論を左右する規模ではないため、緊急対応は不要と判断）。
