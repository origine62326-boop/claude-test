...
| BR004 | DS004由来の正式バックテスト結果（EXP-002, 実MT4レポート+Expertsログ） — PF0.76・純損益-6126.05・最大DD7.45%等 | CC002(Phase5-1, `claude/ea-v0.3.0-risk-management`) | `13fc725a6a6e96fe2dd13af3970e88eeee821d73` | `claude/ea-v0.3.0-risk-management`（未マージ） | **REPRODUCIBLE**（2026-07-31） | `trading-system/reports/raw/EXP-002_run001_report.htm`(DS004, checksum記録済み)、Expertsログ`c4c81a4a-EXP002_run01_experts.log.log`(cp932)、修正後の`analysis/parse_mt4_report.py`・`analysis/parse_mt4_trades.py`・`analysis/common.py` | 未割当 | 2026-07-31。レポート由来の総取引数196件と、Expertsログ由来のopen件数196件が一致することを確認し、両ファイルが同一バックテスト実行を指すことをクロスチェック済み（`EXP-002`「Expertsログ解析結果の反映」節参照） | DS004 | EXP-002 (COMPLETED, HOLD) |
| BR005 | DS005由来の正式バックテスト結果（EXP-003, 日次損失上限機能試験、実MT4レポート+Expertsログ） — PF0.75・純損益-6390.68・最大DD7.71%等（非標準パラメータ実行、収益性比較には不使用） | CC002(Phase5-1, `claude/ea-v0.3.0-risk-management`) | `13fc725a6a6e96fe2dd13af3970e88eeee821d73` | `claude/ea-v0.3.0-risk-management`（未マージ） | **REPRODUCIBLE**（2026-07-31） | `trading-system/reports/raw/EXP-003_run001_report.htm`(DS005, checksum記録済み)、Expertsログ`EXP003_run001_experts.log.log`(cp932)、`analysis/parse_mt4_report.py`・`analysis/common.py`（操作履歴側パーサーは本データでは不使用、操作履歴`.htm`自体が本MT4環境で取得不可のため） | 未割当 | 2026-07-31。レポート由来の総取引数194件と、Expertsログ由来の「発注成功」ログ194件（ticket 1〜194連番・重複なし）が一致することを確認し、両ファイルが同一バックテスト実行を指すことをクロスチェック済み（`EXP-003`「実行結果」節参照） | DS005 | EXP-003 (COMPLETED, decision=ADOPTED〔実験単体〕) |
