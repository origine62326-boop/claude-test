...
| CC005 | Python解析コード全般 — `trading-system/analysis/`, `trading-system/scripts/` | v0.2.0 | `d1705dbbc3fd0295b11894b67e18a612ede39f1e` | `main` | ACTIVE | `beautifulsoup4>=4.12.0`, `lxml>=5.0.0`, `PyYAML>=6.0`, `pytest>=8.0.0`（`trading-system/requirements.txt`） | 未割当 | **2026-07-28、本registry作成時に`pytest tests/ -q`を実行し36 passed（0.53s）を実測で確認**（環境: Python 3.11.15, pytest 9.1.1）。実MT4出力ファイルでの検証は引き続き未実施 | DS001(未確定), `tests/fixtures/`内のサンプルレポート（`DATASET_REGISTRY.md`には未登録） | なし |
| CC006 | MT4レポート解析（CC005のサブセット） — `analysis/parse_mt4_report.py`, `analysis/parse_mt4_trades.py` | v0.2.0 | `d1705dbbc3fd0295b11894b67e18a612ede39f1e` | `main` | ACTIVE | CC005に同じ（beautifulsoup4, lxml） | 未割当 | 2026-07-28（CC005の一部として同一pytest実行で確認。fixtureベースのみで実MT4出力ファイルでの検証はTODO継続中） | DS001(未確定), DS002(UNVERIFIED_OBSERVATION、未投入) | なし |
