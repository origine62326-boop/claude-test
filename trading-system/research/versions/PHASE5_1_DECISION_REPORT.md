...
- 日次損失上限・連敗制限が実際に機能した（狙った通りに新規エントリーを止めた）ことを示すExpertsログの
  ...
  **[2026-07-29追記] 引き続き未解消。`EXP-002`の比較項目7・8として、Phase5-1版バックテスト実行時に
  Expertsログの提供を要請中** → **[2026-07-31追記] Expertsログ受領・解析完了。連敗制限
  (`MaxConsecutiveLosses=3`)は実バックテスト中に2回自然発動し（2026-05-14 17:00, 2026-06-17 19:00）、
  いずれも翌サーバー日付で正常再開・停止中の新規注文0件を確認した。これはTEST_PLAN_PHASE5-1.mdの
  #5（連敗数、ちょうど上限到達）・#7（日付変更後に連敗制限が解除される）と概ね同等の確認が、
  意図的なシナリオ再現ではなく実データ上で自然に得られたことを意味する（`EXP-002`参照）。
  ...
  `MaxDailyLossPercent=0.3`・`MaxConsecutiveLosses=20`でユーザー承認済み、実行完了（DS005）。
  日次損失制限が14回発動、いずれも正常な停止・翌日再開・連敗制限との非干渉を確認し、日次損失上限
  機能も**VERIFIED**となった（`EXP-003`「実行結果」節参照）。Phase5-1の2機能（連敗制限・日次損失
  上限）は、いずれも「コードとして意図通り動作する」ことが確認できた。ただし、これはH002が
  本来評価対象とする既定値パラメータでのリスク管理効果（最大DD・Tail Loss改善等）の実証ではない
  （`EXP-003`は非標準パラメータでの機能試験であり収益性評価には使わない設計）**

## 8. 未解決事項

TEST_PLAN_PHASE5-1.mdの未実施15項目のうち、特に判断材料として重要と考えられるもの:

- ~~#2, #3(日次損失上限の到達・超過時の挙動)~~ **[2026-07-31解消]** `EXP-003`（非標準パラメータでの
  専用機能試験、DS005）で14回の発動を確認。発動後は当日中新規注文が発生しないことも14回すべてで確認済み
- ~~#5(連敗数到達時の挙動)~~ **[2026-07-31解消]** `EXP-002`のExpertsログで2回の自然発動と正常な
  停止・再開を確認済み（連敗制限機能=VERIFIED、上記第7節参照）
- ~~#8(日次損失ラッチの日付変更後の解除)~~ **[2026-07-31解消]** `EXP-003`で14回の発動それぞれについて
  翌暦日以降に発動ログが連続しないことを確認し、日付変更でのラッチ解除を確認済み
- **#9(回復しても当日中は解除されないことの確認)**: `EXP-003`の合格条件3（発動後、当日中は新規注文が
  ない）が14回すべてで確認できたことは、含み損が日中に回復した場合でもブロックが解除されないという
  ラッチ方式の安全設計を強く裏付けるが、「日中に実際に含み損が回復した具体的なケース」を個別に
  特定した検証ではないため、TEST_PLAN_PHASE5-1.md #9が要求する厳密な意味での確認としては
  **部分的に解消**にとどまる（完全な直接確認ではない点に留意）
