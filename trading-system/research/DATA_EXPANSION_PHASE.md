...
9. Effect Size が検出可能な水準かを評価可能にする
   → **[2026-08-25 完了] `DETECTABLE_EFFECT_SIZE_ASSESSMENT.md`。結果は否定的:
   10年分でも検出できるのは +0.0834R 以上（多重検定補正込みなら +0.1104R 以上）で、
   現在追っている +0.02〜+0.06R は原理的に検証できない。下記「必要なデータ期間」の注記を参照**
...
**[2026-08-25 重要な制約の追記]** 完了条件⑨の評価（`DETECTABLE_EFFECT_SIZE_ASSESSMENT.md`）により、
10年（買い約1,338件）で検出できる最小効果量は **+0.0834R**（Bonferroni補正込みで +0.1104R）
であることが判明した。**10年は「+0.08R以上の効果しか検証できない量」である。**
+0.05R を検出するには買い3,725件 ≈ 28年が必要で、現実的に調達できない。
したがって本フェーズは「10年を集めれば既存仮説を検証できる」フェーズではなく、
**「検証できる効果量の下限を確定し、それ未満の仮説を追わないと決めるためのフェーズ」**
として位置づけを改める。データ取得目標そのもの（10年）は変更しない。
