...
例（プロジェクト非依存の一般例。実際に本プロジェクトへ登録されたHypothesis IDとは対応しない）:
...
現在検討中の経済指標フィルターは、
最初から有効な機能として扱わず、
H003（`research/hypotheses/HYPOTHESIS_REGISTRY.md`に登録する
「重要経済指標前後の新規エントリー停止によるPF/最大DD改善仮説」）
の検証対象として登録すること。

> 【2026-07-28改訂メモ】初版では上記の一般例がH001〜H004という具体的なIDで例示されており、
> 経済指標前後停止をH004として登録するよう指示していたが、`HYPOTHESIS_REGISTRY.md`側の実際の
> 初期登録指示ではH001〜H003として別内容が登録される設計だったため、経済指標前後停止の参照先が
> H004(本節)とH003(レジストリ)で一致しない状態だった。PR #8のレビューで指摘を受け、本節の例を
> プロジェクト非依存の一般例（例A〜例C）に改め、経済指標前後停止の参照をH003に統一した。
...
### 2026-07-28 改訂1（PR #8レビュー対応）

- **変更理由**: PR #8のレビューで、第5節の例示（一般例）と`HYPOTHESIS_REGISTRY.md`の実登録との間で、
  経済指標前後の新規エントリー停止のHypothesis IDが一致しない（憲章側=H004、レジストリ側=H003）
  という指摘を受けた。
- **差分**: 第5節の例示をH001〜H004という具体的ID表記から、プロジェクト非依存の一般例（例A〜例C）
  へ変更し、経済指標前後停止の参照をH003へ統一した。第5節本文自体（本作業前の状態）は本改訂の
